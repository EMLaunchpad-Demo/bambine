...
(function () {
  ...
  $$(".opt").forEach(function (opt) {
    opt.addEventListener("click", function () {
      var group = opt.closest("[role=tabpanel]") || document;
      $$(".opt", group).forEach(function (o) { o.setAttribute("aria-pressed", o === opt ? "true" : "false"); });
      chosen = {
        name: opt.dataset.name || "",
        price: opt.dataset.price || "",
        duration: opt.dataset.duration || ""
      };
      var target = $("#booking-summary");
      if (target) target.textContent = chosen.name + " · " + chosen.duration + " · " + chosen.price;
    });
  });

  /* 6. Boekingsdialoog (demo) ---------------------------------------------- */
  var dlg = $("#book-dialog");
  if (dlg) {
    $$("[data-book]").forEach(function (btn) {
      btn.addEventListener("click", function (e) {
        e.preventDefault();
        var pick = btn.dataset.book;
        var sel = $("#f-dienst");
        if (sel) {
          var wanted = (chosen && chosen.name) || (pick && pick !== "true" ? pick : null);
          if (wanted) {
            $$("option", sel).forEach(function (o) {
              if (o.value.toLowerCase().indexOf(wanted.toLowerCase().slice(0, 12)) === 0) sel.value = o.value;
            });
          }
        }
        if (typeof dlg.showModal === "function") dlg.showModal();
        else window.location.href = "contact.html#afspraak";
      });
    });
    $$("[data-dialog-close]", dlg).forEach(function (b) {
      b.addEventListener("click", function () { dlg.close(); });
    });
    dlg.addEventListener("click", function (e) { if (e.target === dlg) dlg.close(); });
  }

  /* 7. Formulieren (demo — geen backend) ----------------------------------- */
  $$("form[data-demo-form]").forEach(function (form) {
    form.addEventListener("submit", function (e) {
      e.preventDefault();
      if (!form.reportValidity()) return;
      var ok = $("[data-form-ok]", form.closest(".dialog-in, .form-wrap") || document);
      if (ok) {
        ok.hidden = false;
  ...
  /* 10. Stappen: voortgangslijn + actieve stap ------------------------------ */
  var steps = $(".steps");
  if (steps && !reduced) {
  ...
  /* 11. Zachte parallax in de galerij --------------------------------------- */
